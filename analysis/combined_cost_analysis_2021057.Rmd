...
data1 = joined %>%  
  ...
  #makes new meter bin and fixes some strings and factor levels
...
data2_age = data1 %>% 
  ...
  summarise(median = median(value, na.rm = T), 
            ...
            sqrtN = sqrt(n()),
            semd = 1.253*sd/sqrtN) %>%  
  pivot_wider(values_from = median:semd, names_from = "name") %>% 
  mutate(`median_C+P Cost` = `median_Preventative Cost` + `median_Corrective Cost`) %>%  
  pivot_longer(cols = contains("Cost"), 
...
Key takeaways (20210507): 

* Appears that age doe some insight on when to 
